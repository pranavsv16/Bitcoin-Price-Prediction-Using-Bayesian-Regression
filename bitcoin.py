...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(Xi)):
      ...
      std_b = np.std(x_val)
      mean_a = np.mean(X[:-1])
      mean_b = np.mean(x_val)
      inside_num = 0
      count = 0
      for j in range(len(x_val)):
        inside_num += (x_val[j]-mean_b)*(X[:-1][j]-mean_a)
        count +=1
      similarity = np.exp((wt*inside_num)/(std_a*std_b*count))
      numerator.append(y_val*similarity)
      denominator.append(similarity)
    expectation = sum(numerator)/sum(denominator)
    return expectation
    pass
